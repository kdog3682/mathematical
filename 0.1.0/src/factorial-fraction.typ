...
#let factorial-fraction(top: none, bottom: none, delimiter: "times", cancel: false, lhs: none, highlight-remaining: false, highlight-color: "blue", centered: false, ..sink) = {
    ...
    let runner(n, opts, cancels) = {
        ...
        let hide = opts.at("hide", default: false)
        if hide == true {
            return hidden(5pt)
        }
        return colored(factorial(n, 
        cancels: cancels, 
        limit: 10, 
        left: opts.left, 
        right: opts.right, 
        highlight-remaining: highlight-remaining), opts.fill)
    }
    ...
    let value = rhs
    if lhs != none {
        let lhs = fraction(colored(str-add(a, "!"), top.fill), colored(str-add(b, "!"), bottom.fill))
        value = (lhs, rhs).join(marks.math.equals)
    }
    if centered == true {
        set text(size: 1.5em)
